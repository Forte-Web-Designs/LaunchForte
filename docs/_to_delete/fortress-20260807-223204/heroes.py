...
The library is a reservoir: 374 shots so that ANY scenario has something.
...
"stalled-deal-escalation": dict(
  ...
  beats=["Here's the system that watches for a deal that hasn't moved and escalates before it goes cold.",
         ...
         "That build is in GoHighLevel and this one's in HubSpot, because that's where my last two clients lived. The tool changes; the architecture doesn't.",
...
def main():
    ...
    w("**Never attach more than four.** The library has 374 shots so that any scenario is covered;")
    ...
    for shape, spec in SHAPES.items():
        ...
        result = (pick(rest, h4) if h4 else None) or \
                 pick(rest, "board", "list", "record", "dashboard", "saved", "detail",
                      "variance", "summary", "report") or (rest[0] if rest else None)
