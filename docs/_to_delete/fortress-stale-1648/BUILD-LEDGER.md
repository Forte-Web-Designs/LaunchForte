...
## G. STATUS — 2026-08-07, end of the build-out pass

| | |
|---|---|
| Screenshots | **310** across 6 tools, 22 shape folders |
| n8n kits | **17** — one per shape, 18 nodes each, all saved as unpublished drafts |
| Tools with proof | n8n 54 · GoHighLevel 78 · HubSpot 49 · Shopify 33 · Stripe 13 · QuickBooks 12 |
| Docs location | `~/repos/launchforte/docs/fortress/` — readable by Fortress before any build |

**New access opened this pass.** Shopify development store (free, Plus plan, self-seeded) at
`launch-forte-demo-store.myshopify.com` · Stripe test mode with 4 customers, 4 products,
5 payments, 4 invoices including one partially paid and one unpaid · QuickBooks sandbox
"Sandbox Company US 7f63", pre-populated, nothing created.

**GoHighLevel unblocked.** Seth's existing agency user was assigned to the Launch Forte
sub-account from the agency side, which sends no invitation email. That released calendar
creation and round-robin assignment, both of which are now built.

**New gotchas learned this pass**
- Stripe's API keys page renders the **test secret key in plaintext by default** — no reveal
  click required. Mask it in the DOM before screenshotting that page, always.
- Stripe rejects a backdated `due_date` outright ("expects a timestamp in the future"), so an
  overdue invoice cannot be forced without a test clock. Use `days_until_due=0` and let it age.
- Stripe's "Record off-Stripe payment" locks the amount to the full balance. To get a partially
  paid invoice, create a smaller payment and apply it to the invoice instead.
- Stripe's customer list serves stale data after a create — re-read before deciding a create failed.
- QuickBooks sandbox 404s on `/app/apps` and `/app/appcenter`. Use Integration transactions instead.
- Shopify's public `apps.shopify.com` is outside the extension's granted domains; install
  first-party apps from inside the admin at `/store/<store>/apps/<handle>`.
- Shopify Flow can report "not available on your plan" plus a 500 immediately after install. It is
  transient — reload before believing it.
- Shopify draft orders are not sales, so Analytics legitimately reads $0.00. Do not fake it.
- n8n: the repeating-transform framing trick goes stale when Vue re-renders. Inject a stylesheet
  rule `.vue-flow__transformationpane{transform:… !important}` instead — it beats the inline style.
- n8n: the tool channel can silently corrupt single characters in a large pasted payload. Send
  gzip+base64 and verify a checksum **in-page** before importing, or a kit imports silently wrong.

**Still open**
1. The Cockpit matcher — shape-first resolution with cross-tool fallback. The library is inert
   until this fires. Highest leverage item remaining.
2. 71 frames in `_unclassified/` need a labelling pass before they can be shipped.
3. Zapier, Make, Airtable, Xero, Twilio, Salesforce — still zero proof, all free to open.
