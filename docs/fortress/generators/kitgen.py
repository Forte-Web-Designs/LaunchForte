...
KITS = {
    ...
    "project-ops": dict(
        title="LF DEMO: The board that tells the truth", demand=58,
        story="Keeps board relations honest, standardises the work breakdown, and makes capacity visible before someone drowns.",
        trigger="A work item changes", required=["item_id", "board"], dedupe_key="item_id",
        lookup="Read the linked board and its relations",
        lookup_url="https://example.com/api/board",
        unreachable="Linked board unreachable, flag the relation",
        decision="Does every relation still point somewhere real?",
        decision_expr="={{ $json.relationsValid === true }}",
        action_yes="Roll the status up to the parent",
        action_no="Flag the broken relation, do not silently drop it",
        action_url="https://example.com/api/notify",
        digest="Who is over capacity this week"),

    "ai-assistant": dict(
        title="LF DEMO: The assistant that refuses to guess", demand=127,
        story="Answers from a real knowledge base, refuses when it does not know, and hands to a human with the context attached.",
        trigger="A question arrives", required=["conversation_id", "question"],
        dedupe_key="conversation_id",
        lookup="Retrieve the grounding passages",
        lookup_url="https://example.com/api/knowledge",
        unreachable="Model unreachable, hand to a human now",
        decision="Is the answer actually grounded in a source?",
        decision_expr="={{ $json.grounded === true }}",
        action_yes="Answer, and cite where it came from",
        action_no="Refuse, and pass it to a human with context",
        action_url="https://example.com/api/reply",
        digest="Questions it could not answer, which is the next training set"),

    "cold-outreach": dict(
        title="LF DEMO: The outreach that protects the domain", demand=52,
        story="Validates the list, paces the sending, and stops the sequence the moment someone replies.",
        trigger="A campaign step is due", required=["lead_id", "step"], dedupe_key="lead_id",
        lookup="Verify the address before spending reputation on it",
        lookup_url="https://example.com/api/verify",
        unreachable="Verifier down, hold the send rather than risk the domain",
        decision="Safe to send, and have they stayed quiet?",
        decision_expr="={{ $json.deliverable === true && $json.replied !== true }}",
        action_yes="Send the next step",
        action_no="Stop the sequence, hand to a human",
        action_url="https://example.com/api/send",
    ...
    "messaging-compliance": dict(
        title="LF DEMO: The send that cannot get the domain banned", demand=96,
        story="Checks the sending identity is registered and healthy before the message leaves, because a blocked number or a burnt domain is found weeks late.",
        trigger="A message is queued to send", required=["message_id", "brand_id", "channel"],
        dedupe_key="message_id",
        lookup="Check the brand registration and domain health",
        lookup_url="https://example.com/api/sending-identity",
        unreachable="Registry unreachable, hold the send",
        decision="Is this identity cleared to send right now?",
        decision_expr="={{ $json.registered === true }}",
        action_yes="Release it on the compliant sender",
        action_no="Hold it and tell the owner what is missing",
        action_url="https://example.com/api/send",
        digest="Which brands are registered, and which are drifting"),

    "data-model-architecture": dict(
        title="LF DEMO: The record that fits the schema", demand=88,
        story="Normalises an inbound record to the agreed data model and stops the field nobody mapped from quietly inventing a new one.",
        trigger="A record arrives from a source", required=["record_id", "object_type", "source"],
        dedupe_key="record_id",
        lookup="Read the schema this object is supposed to match",
        lookup_url="https://example.com/api/schema",
        unreachable="Schema unreachable, do not guess the mapping",
        decision="Does every field map to a defined property?",
        decision_expr="={{ $json.fullyMapped === true }}",
        action_yes="Write it to the mapped properties",
        action_no="Quarantine it and ask for the mapping decision",
        action_url="https://example.com/api/records",
        digest="Unmapped fields, and how often each one arrived"),

    "ai-research-agent": dict(
        title="LF DEMO: The research that cites itself", demand=64,
        story="Runs a real research pass, keeps the sources, and refuses to state anything it cannot point at.",
        trigger="A research question arrives", required=["question_id", "question"],
        dedupe_key="question_id",
        lookup="Search the live web for sources",
        lookup_url="https://example.com/api/search",
        unreachable="Search unreachable, do not answer from memory",
        decision="Is every claim backed by a source we fetched?",
        decision_expr="={{ $json.allClaimsCited === true }}",
        action_yes="Write the brief with citations attached",
        action_no="Return what is known and name the gap",
        action_url="https://example.com/api/brief",
        digest="Questions answered, and how many needed a human"),
}
