...
const __p = (function () {
  // ONE implementation of the arithmetic, not two. "Price the build" is the
  // node pricing-check.py actually tests; this used to carry a second, simpler
  // copy of the same maths that nothing tested and that silently disagreed
  // with it. Read the node when it is upstream. The inline derivation below
  // stays ONLY as the fallback for a branch where the node is not wired, so a
  // missing node degrades to a number instead of to an exception.
  try {
    const n = $('Price the build').first().json;
    if (n && n.priced && typeof n.quote_total === 'number') {
      return {
        k: n.pricing_class,
        wf: n.workflows_counted,
        sn: n.systems_counted,
        tot: n.quote_total,
        lines: (n.quote_lines || []).map(function (l) {
          return '- ' + l.label + ': $' + (l.amount || 0).toLocaleString('en-US') + '. ' + l.why;
        }).join('\n'),
        ph: n.pricing_phase_split || null,
        anchor: n.pricing_anchor,
        src: 'node'
      };
    }
  } catch (e) { /* node not upstream on this branch — fall through */ }
  const j = $json || {};
...
const UPWORK_RULES = [
  ...
  'The live demo is offered in the chat after they reply, never in the proposal.',
