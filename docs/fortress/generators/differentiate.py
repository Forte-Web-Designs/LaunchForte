...
"project-ops": dict(
  ceiling=(40, "Board APIs throttle per account, and a rollup touches many items at once"),
  log="itemId: $json.item_id, board: $json.board, relationsValid: $json.relationsValid, owner: $json.owner",
  digest="""// Who is over capacity this week — the number a manager cannot see by eye.
...
return [{ json: { rows, note: drowning.length
  ...
  decide="={{ $json.relationsValid === true && $json.board != null }}"),

"ai-assistant": dict(
  ceiling=(20, "Every question costs a model call, so the ceiling is a cost control as much as a rate limit"),
  log="conversationId: $json.conversation_id, grounded: $json.grounded, confidence: $json.confidence, escalated: $json.escalated",
  digest="""// Questions it could not answer — which is precisely the next training set.
const asked = Object.values($getWorkflowStaticData('global').questions || {});
const week = asked.filter(q => q.at > Date.now() - 7*864e5);
const refused = week.filter(q => !q.grounded);
const byTopic = {};
for (const q of refused) byTopic[q.topic] = (byTopic[q.topic] || 0) + 1;
const worst = Object.entries(byTopic).sort((a,b) => b[1]-a[1])[0];
return [{ json: {
  asked: week.length,
  refused: refused.length,
  note: worst
    ? `"${worst[0]}" was refused ${worst[1]} times. Add that to the knowledge base and the refusals stop.`
    : 'It answered everything it was asked, from source.',
} }];""",
  decide="={{ $json.grounded === true && $json.confidence > 0.75 }}"),

"cold-outreach": dict(
  ceiling=(15, "Sending pace IS the deliverability strategy — a high ceiling burns the domain"),
  log="leadId: $json.lead_id, step: $json.step, deliverable: $json.deliverable, replied: $json.replied",
  digest="""// Bounce rate and reply rate, before the domain pays for it.
const sends = Object.values($getWorkflowStaticData('global').sends || {});
const week = sends.filter(s => s.at > Date.now() - 7*864e5);
const bounced = week.filter(s => s.bounced);
const replied = week.filter(s => s.replied);
const bounceRate = week.length ? (bounced.length / week.length * 100) : 0;
return [{ json: {
  sent: week.length,
  bounceRate: Math.round(bounceRate * 10) / 10,
  replyRate: week.length ? Math.round(replied.length / week.length * 1000) / 10 : 0,
  note: bounceRate > 3
    ? `Bounce rate ${bounceRate.toFixed(1)}% — above 3% the domain starts paying for it. Pause and reverify the list.`
    : 'Bounce rate is inside a safe band. Keep the pace.',
} }];""",
  decide="={{ $json.deliverable === true && $json.replied !== true && $json.step <= 3 }}"),
...
"messaging-compliance": dict(
  ceiling=(12, "Carriers judge a number by its pace, so the ceiling here is the registration staying good"),
  log="messageId: $json.message_id, brandId: $json.brand_id, channel: $json.channel, registered: $json.registered, held: $json.held || false",
  digest="""// Which brands are registered, and which are drifting — before a carrier decides for us.
...
return [{ json: {
  ...
  decide="={{ $json.registered === true && $json.throughputRemaining > 0 }}"),

"data-model-architecture": dict(
  ceiling=(40, "Schema writes are cheap, but a bad mapping replicated at speed is expensive to undo"),
  log="recordId: $json.record_id, objectType: $json.object_type, source: $json.source, fullyMapped: $json.fullyMapped, quarantined: $json.quarantined || false",
  digest="""// Unmapped fields, and how often each one arrived — the schema gap, ranked.
const recs = Object.values($getWorkflowStaticData('global').records || {});
const week = recs.filter(r => r.at > Date.now() - 7*864e5);
const counts = {};
for (const r of week) for (const f of (r.unmapped || [])) counts[f] = (counts[f] || 0) + 1;
const ranked = Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 5);
const quarantined = week.filter(r => r.quarantined);
return [{ json: {
  received: week.length,
  quarantined: quarantined.length,
  worstFields: ranked,
  note: ranked.length
    ? `"${ranked[0][0]}" arrived ${ranked[0][1]} times with nowhere to go. Decide the property once and ${quarantined.length} quarantined records clear themselves.`
    : 'Every field arriving maps to a defined property.',
} }];""",
  decide="={{ $json.fullyMapped === true && $json.objectExists === true }}"),

"ai-research-agent": dict(
  ceiling=(8, "Every question is several model calls and several fetches, so the ceiling is a spend control first and a rate limit second"),
  log="questionId: $json.question_id, sources: ($json.sources || []).length, allClaimsCited: $json.allClaimsCited, escalated: $json.escalated",
  digest="""// Questions answered, and how many needed a human — the honest scorecard.
const asked = Object.values($getWorkflowStaticData('global').research || {});
const week = asked.filter(q => q.at > Date.now() - 7*864e5);
const uncited = week.filter(q => !q.allClaimsCited);
const thin = week.filter(q => (q.sources || 0) < 3);
const avgSources = week.length
  ? Math.round(week.reduce((n, q) => n + (q.sources || 0), 0) / week.length * 10) / 10
  : 0;
return [{ json: {
  asked: week.length,
  handedToAHuman: uncited.length,
  avgSources,
  note: thin.length
    ? `${thin.length} answers rested on fewer than three sources. That is where a research agent starts sounding confident and being wrong.`
    : 'Every answer this week was grounded in at least three fetched sources.',
} }];""",
  decide="={{ $json.allClaimsCited === true && $json.sources.length >= 3 }}"),
}
